...
def test_guided_turn_returns_addressable_tokens_without_emphasis(monkeypatch) -> None:
    ...
    assert anchors
    assert all(anchor.label is None for anchor in anchors)
    assert question[variable.char_start : variable.char_end] == "n"
