...
async def run_tutor_pipeline(
    ...
    """Run the student-model assessment and tutor evaluation sequence."""
...
async def process_answer_with_session_event(
    ...
    """Evaluate one answer and persist its Schema 3.0 learning event."""
    ...
    schema_event_type: Literal["CORRECT_ATTEMPT", "INCORRECT_ATTEMPT"] | None = (
        ...
        or (configured_guided_event is None and tutor.evaluation == "CORRECT")
        else (
            "INCORRECT_ATTEMPT"
            if configured_guided_event == "INCORRECT_ATTEMPT"
            or (
                configured_guided_event is None
    ...
    if schema_managed and schema_event_type is not None:
                ...
                IndependentRetryCompletedEvent(
                    ...
                    error_code=(
                        (
                            ...
                            if tutor.guided_student_state is not None
                            else _db_error_code(session, context.message)
                            or tutor.error_type
                        )
        ...
        elif schema_event_type is not None:
            schema_response = await adapters.student_model.send_session_event(
                GuidedAttemptEvent(
                    request_id=_schema_interaction_request_id(
    ...
    return student, tutor, schema_response, session
...
async def _process_interaction(
    ...
    student, tutor, schema_response, session = await process_answer_with_session_event(
        context,
        session,
        access_token,
    )
    tutor = tutor.model_copy(update={"safety_check": safety_check})
    schema_content_response: StudentModelSessionEventResponse | None = schema_response
    ...
    # Student Model state is the phase authority after the Schema 3.0 event.
    ...
    next_hint_count: int = session.hint_count
