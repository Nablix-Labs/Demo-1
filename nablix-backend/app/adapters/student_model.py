...
class StudentModelServiceAdapter:
    ...
    async def fetch_prerequisite_route(
        ...
        TC-30. Curriculum facts only -- it decides nothing. Student Model
        escalates a third checkpoint failure with
        CHECK_PREREQUISITE_REMEDIATION_ROUTE and then waits: it cannot tell "no
        route exists" from "nobody looked" until this answer comes back as
        PREREQUISITE_ROUTE_RESOLVED, so an empty chain is a real answer and is
        still reported.

        Service-authenticated like the Phase 4 reads: the endpoint is
        internal_service-only (mathtutor-student api/routers/curriculum.py) and
        has no student branch.
        """
