...
**Where the id actually comes from.** Not the Student Model, and not a mock —
it is a real row in **Aditya's Qdrant question bank** (`math_tutor_questions`),
seeded from:
...
nablix-backend/app/services/rag/question_serving/question_bank.json:87
  { "question_id": "ALG_1STEP_GP_F01",
    ...
    "phase": "GUIDED_PRACTICE", "difficulty": "FOUNDATION" }
```

read by `app/adapters/question_bank.py` → `fetch_question()` →
`get_next_question()` (`session_service.py:140`).

**The exact trigger.** On a correct answer in guided practice:

```python
# interaction_service.py:1067
elif conversation_action == "ADVANCE_TO_NEXT_QUESTION" and schema_response is None:
    advance = await next_question_updates(session, session.current_phase)
```

When the Student Model returns no new event (`schema_response is None`), the
Tutor Backend advances to the next question **from the Qdrant bank** and
overwrites `session.question_id` — while `session.student_model_event` still
holds the previous Schema 3.0 `question_set`.

The next answer then hits `_schema_question()`, which looks that id up in the
Student Model's set:
...
Not found → 409, blaming the Student Model for an id that came from Aditya's
bank.

**Observed sequence, 29 July** (`SESSION015`), which matches the code exactly:

1. Student answers correctly → tutor replies *"Exactly. Let us move to the next
   question."* (`ADVANCE_TO_NEXT_QUESTION`)
2. Question changes to `Solve for x: x + 6 = 10`, `question_id` becomes
   `ALG_1STEP_GP_F01` — pulled from the Qdrant bank
3. Student answers again → `409 Student Model did not return metadata for
   ALG_1STEP_GP_F01` (`request_id REQCCC339F4`), and every answer after it

So the question the student is looking at is real and correctly served. What is
broken is that it is then validated against a source that was never going to
contain it.
