...
// One definition of how long a mark takes to write, shared with the hold in
// lib/revealBeforeClear. They used to be two different numbers, which is what
// cut the tutor off mid-word on the way to the next question (#304).
import { elementWritingMs, prefersReducedMotion, GAP_MS } from '@/lib/tutorWritingTime';
    ...
    sync: (elements) => {
      ...
      for (const el of elements) {
        if (next[el.id] === undefined && !queued.has(el.id)) {
          queue.push({ id: el.id, duration: elementWritingMs(el) });
          additions[el.id] = 0;
        }
      }
