 ...
 * commit while the tutor is still writing, when the turn both annotated the finished question and
...
describe('the phase-mutation seam', () => {
  ...
  it('still holds the phase change back while the marks are read', () => {
    ...
    const held = revealDecision(1500, 'Q1', 'Q2');
    expect(held).toEqual({ reveal: true, holdMs: 1500 });
    // ...and no hold when there is nothing to reveal, so ordinary turns are
    // not delayed.
    expect(revealDecision(0, 'Q1', 'Q2').holdMs).toBe(0);
    expect(revealDecision(1500, 'Q1', 'Q1').holdMs).toBe(0);
  });
});
