...
//
// Numera: the generated preset shipped a plum/rose/cream ramp with a large hue
// rotation. UNIFORMS below carries our palette instead (see the comment there);
// everything above it is the generated shader, unedited.
...
// Numera palette. The generated preset ramped plum → rose → cream and then spun
// it with a 130° hue rotation; both are replaced here.
//
// Content on these screens is near-black on the existing white background, so
// the ramp stays pale — white at the top through to a soft brand blue at the
// bottom — and `hue` is zeroed so the colours below are the colours rendered.
// `saturation` drops from 2.0 for the same reason: at 2.0 the blues turned
// electric and fought the text sitting on top of them.
//
// Everything else (scale, warp, detail, rotate, drift, seed, timeScale) is the
// generated preset untouched, so the movement is the one that was designed.
const UNIFORMS = {
  colors: [
    [0.4392156862745098, 0.6431372549019608, 0.8862745098039215], // #70A4E2
    [0.7568627450980392, 0.8549019607843137, 0.9568627450980393], // #C1DAF4
    [0.9294117647058824, 0.9568627450980393, 0.9882352941176471], // #EDF4FC
    [1, 1, 1],                                                    // #FFFFFF
    [1, 1, 1],
    [1, 1, 1],
    [1, 1, 1],
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  /**
   * Bumped to re-run setup after the GPU takes the context away.
   *
   * Numera runs several WebGL contexts at once — this background, the 3D tutor
   * avatar, the shader on the pre-auth panel — and browsers cap how many can
   * live at a time, evicting the oldest. A canvas whose context is gone keeps
   * whatever pixels it last held and silently ignores every draw call, which is
   * how these screens turned dark mid-session (Manav, 2026-07-29). Nothing here
   * listened for that, so it never recovered.
   */
  ...
  useEffect(() => {
    ...
    const onLost = (e: Event) => {
      e.preventDefault()
      console.warn("[shader] WebGL context lost; holding the light background")
    }
    ...
    if (!gl) return detachContextListeners

    // A student who has asked for reduced motion gets the same image, held
    // still: time stops advancing, so the waves render once and stay put.
    ...
    function render(now: number) {
      raf = 0
      // canvas/gl are re-checked because `render` is a hoisted declaration, so
      // the narrowing from the guards above doesn't reach inside it.
