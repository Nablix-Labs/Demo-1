...
export interface DockApp {
  ...
  name: string;
  /**
   * An image URL (the original behaviour) or a rendered element — an inline
   * SVG icon, a lucide glyph, anything. Elements are stretched to fill the
   * tile, so they should be square and sized in percentages.
   */
  icon: string | React.ReactElement;
  /**
   * When present the item renders as a real link. Icons were `<div onClick>`,
   * which cannot be tabbed to or opened in a new tab; as app navigation that
   * is a dead end for keyboard and screen-reader users.
   */
  href?: string;
}
...
interface MacOSDockProps {
  ...
  className?: string;
  /**
   * Class applied to the dock surface. When set, the component drops its own
   * inline background and border so the class can own the material (e.g.
   * `lg-glass-dark`). Left unset, the default macOS-style dark chrome applies.
   */
  surfaceClassName?: string;
  /** Floating name above the hovered icon, the way macOS labels a Dock item. */
  showLabels?: boolean;
  'aria-label'?: string;
}
...
const MacOSDock: React.FC<MacOSDockProps> = ({
  ...
  const padding = Math.max(8, baseIconSize * 0.12);
  const themed = surfaceClassName.length > 0;

  return (
    <nav
      ref={dockRef}
      aria-label={ariaLabel}
      className={`backdrop-blur-md ${surfaceClassName} ${className}`}
      style={{
        ...
        `,
        padding: `${padding}px`,
        // A themed surface supplies its own material via surfaceClassName.
        ...(themed ? null : {
    ...
    >
      ...
      >
        {apps.map((app, index) => {
          ...
          const shadow = `drop-shadow(0 ${scale > 1.2 ? Math.max(2, baseIconSize * 0.05) : Math.max(1, baseIconSize * 0.03)}px ${scale > 1.2 ? Math.max(4, baseIconSize * 0.1) : Math.max(2, baseIconSize * 0.06)}px rgba(0,0,0,${0.2 + (scale - 1) * 0.15}))`;
          ...
          // The label floats above the dock, so it must not be clipped by the
          // icon box or reordered under a neighbouring tile.
          const label = showLabels && hovered === index && (
            <span
              className="absolute left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md px-2 py-1 text-white pointer-events-none"
              style={{
                bottom: `${scaledSize + padding * 1.4}px`,
                fontSize: `${Math.max(11, baseIconSize * 0.17)}px`,
                background: 'rgba(18, 27, 51, 0.82)',
                border: '1px solid rgba(255,255,255,0.18)',
                boxShadow: '0 6px 18px rgba(11,16,32,0.45)',
          ...
          const boxClass =
            'absolute cursor-pointer flex flex-col items-center justify-end rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-white/80';

          const shared = {
            ref: (el: HTMLElement | null) => { iconRefs.current[index] = el; },
            className: boxClass,
            style: boxStyle,
            'aria-label': app.name,
            'aria-current': (isOpen && app.href ? 'page' : undefined) as 'page' | undefined,
          ...
          return app.href ? (
            <Link key={app.id} href={app.href} {...(shared as any)}>
              {artwork}
              {dot}
              {label}
            </Link>
          ) : (
            <button key={app.id} type="button" {...(shared as any)}>
              {artwork}
              {dot}
              {label}
            </button>
